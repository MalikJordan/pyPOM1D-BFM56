...
from bfm.bfm56.Functions.other_functions import eTq_vector, get_concentration_ratio, insw_vector
...
def bacteria_eqns(d3state, bacteria_parameters, constant_parameters, environmental_parameters, temper):
    ...
    if bacteria_parameters["bact_version"]==1:
        ...
        dBcdt_rel_r2c = np.zeros(num_boxes)
        for i in range(0,num_boxes):
            dBcdt_rel_r2c[i] = max(1.-(bp_bc[i]/bacteria_parameters["p_B_opt"]) , (1.-(bn_bc[i]/bacteria_parameters["n_B_opt"])*bacteria_parameters["p_rec"]))
