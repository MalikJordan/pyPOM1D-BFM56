...
def mesozoo_eqns(d3state, mesozoo_parameters, constant_parameters, environmental_parameters, zc, zn, zp, i_c, i_n, i_p, temp):
    ...
    # temp_p = pu_e_p/(mesozoo_parameters["p_qpcMEZ"] + constant_parameters["p_small"])
    # temp_n = pu_e_n/(mesozoo_parameters["p_qncMEZ"] + constant_parameters["p_small"])
    for i in range(0,num_boxes):
        limiting_nutrient = 'carbon'
        # # temp_p = pu_e_p[i]/(zp_zc[i] + constant_parameters["p_small"])
        # # temp_n = pu_e_n[i]/(zn_zc[i] + constant_parameters["p_small"])
        # temp_p = pu_e_p[i]/(mesozoo_parameters["p_qpcMEZ"] + constant_parameters["p_small"])
        # temp_n = pu_e_n[i]/(mesozoo_parameters["p_qncMEZ"] + constant_parameters["p_small"])
        ...
        if limiting_nutrient == 'carbon':
            q_Zc[i] = 0.0
            # q_Zp[i] = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_p[i] - mesozoo_parameters["p_Zopt"]*ru_c[i])
            # q_Zn[i] = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_n[i] - mesozoo_parameters["n_Zopt"]*ru_c[i])
            ...
            q_Zp[i] = 0.0
            # q_Zc[i] = max(0.0, ru_c[i] - (1.0 - mesozoo_parameters["betaZ"])*i_p[i]/mesozoo_parameters["p_Zopt"])
            # q_Zn[i] = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_n[i] - mesozoo_parameters["n_Zopt"]*(ru_c[i] - q_Zc[i]))
            ...
            q_Zn[i] = 0.0
            # q_Zc[i] = max(0.0, ru_c[i] - (1.0 - mesozoo_parameters["betaZ"])*i_n[i]/mesozoo_parameters["n_Zopt"])
            # q_Zp[i] = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_p[i] - mesozoo_parameters["p_Zopt"]*(ru_c[i] - q_Zc[i]))
