...
def calculate_oxygen_reaeration(oxygen_reaeration_parameters, environmental_parameters, constant_parameters, d3state, del_z, temper, salt, wind):
    ...
    # oxy_sat = np.exp(-173.4292 + (249.6339/abt) + (143.3483*np.log(abt))-(21.8492*abt) + salt*(-0.033096 + 0.014259*abt - 0.0017*(abt**2)))*44.661
    ...
    # wind_dependency = (oxygen_reaeration_parameters["d"]*(wind**2))*np.sqrt(schmidt_ratio_o2o)*constant_parameters["cm2m"]*constant_parameters["hours_per_day"]/constant_parameters["sec_per_day"]
