...
def phyto_eqns(d3state, bfm_phys_vars, phyto_parameters, env_parameters, constant_parameters, del_z, group, irradiance, pc, pn, pp, pl, suspended_sediments, temp, time, xEPS):
    ...
    # pn_pc = np.zeros(num_boxes)     # N:C
    # pp_pc = np.zeros(num_boxes)     # P:C
    # pl_pc = np.zeros(num_boxes)     # Chl:C
    # for i in range(0,num_boxes):
    #     pn_pc[i] = get_concentration_ratio(pn[i], pc[i], constant_parameters["p_small"])
    #     pp_pc[i] = get_concentration_ratio(pp[i], pc[i], constant_parameters["p_small"])
    #     pl_pc[i] = get_concentration_ratio(pl[i], pc[i], constant_parameters["p_small"])
    ...
    if group == 1:
        ...
        # dPsdt_lys_r6s = 0.0
        dPsdt_upt_n5s = np.zeros(num_boxes)
        # dPsdt_lys_r6s = np.zeros(num_boxes)
        dPsdt_lys_r6s = nut_stress_lysis*p1s
    ...
    if (phyto_parameters["p_res"] > 0): # from Phyto.F90 
        for i in range(0,num_boxes):
            ...
            # sedi[i] = sedi[i] + phyto_parameters["p_res"]*max(0., phyto_parameters["p_esNI"]-tN[i])

    # bfm_phys_vars.phyto_sedimentation[:,group-1] = sedi
    # x=1
    #--------------------------------------------------------------------------
