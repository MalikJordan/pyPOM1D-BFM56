import numpy
import sys
from reduction.PythonBFM.other_functions import insw_vector, eTq_vector, get_concentration_ratio

def phyto_eqns(conc, phyto_parameters, env_parameters, constant_parameters, group, pc, pn, pp, pl, qs, temp, time, xEPS):
    ...
    in1p = min(1.0, max(constant_parameters["p_small"], (pp_pc - phyto_parameters["phi_Pmin"])/(phyto_parameters["phi_Popt"] - phyto_parameters["phi_Pmin"]))) # (from Phyto.F90 'iN1p')
    ...
    tN = min(multiple_nut_lim,fpplim)
   ...
   #-------------------------------------------------------------------------- 
    ...
    irr = max(constant_parameters["p_small"], r * constant_parameters["sec_per_day"])

    exponent = pl_pc*phyto_parameters["alpha_chl"]/phyto_parameters["rP0"]*irr     # Compute exponent E_PAR/E_K = alpha0/PBmax (part of eqn. 2.2.4)

    light_lim = (1.0 - numpy.exp(-exponent))     # light limitation factor (from Phyto.f90 line 374, eiPPY)
    ...
    if phyto_parameters["chl_switch"] == 1:
        ...
        rho_chl = phyto_parameters["theta_chl0"]*min(1.0,(photosynthesis - nut_stress_excretion - activity_excretion - activity_rsp)*pc/(phyto_parameters["alpha_chl"]*(pl + constant_parameters["p_small"])*irr))
        chl_opt = phyto_parameters["p_EpEk_or"]*phyto_parameters["rP0"]*pc/(phyto_parameters["alpha_chl"]*irr+constant_parameters["p_small"])
        # for Phyto2 : chl_opt = 0 & chl_relax = 0 (from Pelagic_Ecology.nml)
        dPldt_syn = rho_chl*(photosynthesis - nut_stress_excretion - activity_excretion - activity_rsp)*pc - (nut_stress_lysis + basal_rsp)*pl - max(0.0,pl-chl_opt)*phyto_parameters["p_tochl_relt"]
    else:
