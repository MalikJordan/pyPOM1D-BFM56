...
def mesozoo_eqns(conc, mesozoo_parameters, constant_parameters, environmental_parameters, zc, zn, zp, i_c, i_n, i_p, temp):
    ...
    if limiting_nutrient == 'carbon':
        q_Zc = 0.0
        q_Zp = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_p - mesozoo_parameters["p_qpcMEZ"]*ru_c)
        q_Zn = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_n - mesozoo_parameters["p_qncMEZ"]*ru_c)
    elif limiting_nutrient == 'phosphorus':
        q_Zp = 0.0
        q_Zc = max(0.0, ru_c - (1.0 - mesozoo_parameters["betaZ"])*i_p/mesozoo_parameters["p_qpcMEZ"])
        q_Zn = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_n - mesozoo_parameters["p_qncMEZ"]*(ru_c - q_Zc))
    elif limiting_nutrient == 'nitrogen':
        q_Zn = 0.0
        q_Zc = max(0.0, ru_c - (1.0 - mesozoo_parameters["betaZ"])*i_n/mesozoo_parameters["p_qncMEZ"])
        q_Zp = max(0.0, (1.0 - mesozoo_parameters["betaZ"])*i_p - mesozoo_parameters["p_qpcMEZ"]*(ru_c - q_Zc))
